      ...
      doPersistedAsync(createRequest) { (msg, originalSender)=>
        ...
        validateNewProjectRequest(createRequest) match {
          case Success(validatedRequest) =>
            ...
            ProjectEntry
              .createFromFile(writtenFile, rq.projectTemplate, rq.title, createTime, rq.user.toLowerCase(), rq.workingGroupId, rq.commissionId, rq.existingVidispineId, rq.deletable, rq.deep_archive, rq.sensitive, rq.productionOffice)
              .map({
