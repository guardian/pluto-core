...
class PostrunExecutorSpec extends Specification with BuildMyApp with Mockito {
  ...
  "PostrunExecutor->NewProjectRollback" should {
    "return StepSucceeded" in new WithApplication(buildApp){
      ...
      val maybeRq = Await.result(ProjectRequest("testprojectfile",1,"Test project entry", 1, "test-user", None, None,deletable=true, deep_archive=false, sensitive=false, ProductionOffice.Aus, None).hydrate, 10 seconds)
      maybeRq must beSome
