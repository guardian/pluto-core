...
class ProjectEntryEditComponent extends React.Component {
  ...
  confirmClicked() {
    ...
    let finalPromise = axios
      .put("/api/project/" + projectId + "/title", {
    ...
    if (
      this.state.vsidChanged &&
      (!this.state.vsidInvalid || this.state.removeVsid)
    )
      finalPromise = finalPromise.then(() =>
        axios
          .put("/api/project/" + projectId + "/vsid", {
            title: this.state.projectName,
            vsid: this.state.removeVsid ? null : this.state.vsid,
          })
          .then((response) => {})
          .catch((error) => {
            console.error(error);
  ...
  componentWillMount() {
    ...
    axios
      ...
      .then((response) =>
        this.setState(
          {
            projectName: response.data.result.title,
            vsid: response.data.result.vidispineId,
          },
          () =>
            this.vsidUpdated({
              target: { value: response.data.result.vidispineId },
            })
        )
      )
  ...
  vsidUpdated(event) {
    this.setState({
      vsidChanged: true,
      vsid: event.target.value,
      vsidInvalid: validateVsid(event.target.value) !== null,
    });
  }
      ...
      <div className="filter-list-block">
        ...
        <table style={{ width: "100%" }}>
          <tbody>
            ...
            <tr>
              <td style={{ width: "20%" }}>
                <label htmlFor="vsid-input">Vidispine ID: </label>
              </td>
              <td>
                <input
                  id="vsid"
                  value={this.state.vsid}
            ...
            <tr>
              <td>
                <label htmlFor="vsid-remove">Remove vidispine ID:</label>
              </td>
              <td>
                <input
                  type="checkbox"
                  id="remove-vsid"
                  value={this.removeVsid}
                  onChange={(event) =>
                    this.setState({ removeVsid: event.target.value })
                  }
                />
              </td>
            </tr>
