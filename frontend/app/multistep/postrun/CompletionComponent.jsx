...
class CompletionComponent extends CommonCompletionComponent {
  ...
  updateAddRemoveDeps() {
    this.setState({
      depsToRemove: this.props.originalDependencies.filter(
        (depId) => !this.props.selectedDependencies.includes(depId)
      ),
      depsToAdd: this.props.selectedDependencies.filter(
        (depId) => !this.props.originalDependencies.includes(depId)
      ),
    });
  }
  ...
  doRemoveDeps() {
    Promise.all(
      this.state.depsToRemove.map((depId) =>
        axios.delete(
          "/api/postrun/" + this.props.currentEntry + "/depends/" + depId
        )
      )
    );
  }
    ...
    return (
      <div>
        ...
        <p className="information">
          The postrun action information will be edited as below. Click Confirm
          to continue or Back to change.
        </p>
        <SummaryComponent
          title={this.props.postrunMetadata.title}
