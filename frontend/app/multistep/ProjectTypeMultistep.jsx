  ...
  loadPostrunActions() {
    ...
    this.setState({ loading: true }, () => {
      ...
      if (this.state.currentEntry)
        promiseList.push(
          axios.get("/api/projecttype/" + this.state.currentEntry + "/postrun")
        );
      ...
      Promise.all(promiseList)
        .then((responseList) => {
          this.setState({
            ...
            selectedPostruns: responseList[1]
              ? responseList[1].data.result
              : [],
            originalPostruns: responseList[1]
              ? Object.assign([], responseList[1].data.result)
              : [],
            loading: false,
          });
        })
    ...
    const steps = [
      {
        ...
        component: (
          <ProjectTypeComponent
            ...
            valueWasSet={(newtype) => {
              this.setState({ projectType: newtype });
            }}
          />
        ),
      },
      ...
      },
      {
        name: "Confirm",
        component: (
          <ProjectTypeCompletionComponent
