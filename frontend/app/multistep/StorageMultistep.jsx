...
class StorageMultistep extends CommonMultistepRoot {
  ...
  loadEntity(entryId) {
    return axios
      .get("/api/storage/" + entryId)
      .then((response) => {
        console.log("Got existing entity: ", response);
        this.setState({
          rootpath: response.data.result.rootpath,
          clientpath: response.data.result.clientpath,
          selectedType: this.getStorageTypeIndex(
            response.data.result.storageType
          ),
          loginDetails: {
            hostname: response.data.result.host,
            port: response.data.result.port
              ? parseInt(response.data.result.port)
              : null,
    ...
    const steps = [
      {
        ...
        component: (
          <StorageTypeComponent
            ...
            versionsAllowedChanged={(newValue) =>
              this.setState({ enableVersions: newValue })
            }
          />
        ),
      },
      ...
      },
      {
        ...
        component: (
          <StorageSubfolderComponent
            ...
            valueWasSet={(values) =>
              this.setState({
                rootpath: values.subfolder,
                clientpath: values.clientpath,
              })
            }
          />
        ),
