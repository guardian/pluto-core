...
class ProjectTypeCompletionComponent extends CommonCompletionComponent {
  ...
  updateAddRemoveDeps() {
    this.setState({
      depsToRemove: this.props.originalPostruns.filter(
        (depId) => !this.props.selectedPostruns.includes(depId)
      ),
      depsToAdd: this.props.selectedPostruns.filter(
        (depId) => !this.props.originalPostruns.includes(depId)
      ),
    });
  }

  recordDidSave() {
    const myId = this.props.currentEntry
    ...
    return Promise.all(
      this.state.depsToAdd.map((depId) =>
        axios.put("/api/postrun/" + depId + "/projecttype/" + myId)
      )
    ).then(
      Promise.all(
        this.state.depsToRemove.map((depId) =>
          axios.delete("/api/postrun/" + depId + "/projecttype/" + myId)
        )
      )
    );
  }
  ...
  requestContent() {
    ...
    if (this.props.projectType.plutoType)
      result.plutoType = this.props.projectType.plutoType;
    if (this.props.projectType.plutoSubtype)
      result.plutoSubtype = this.props.projectType.plutoSubtype;
    ...
    return (
      <div>
        ...
        <p className="information">
          We will set up a new project type definition with the information
          below.
        </p>
        <p className="information">
        ...
        />
        <p className="information">
          <b>Note:</b> if you have set a Pluto type and/or subtype identifier,
          there can only be one instance of each type and subtype combination in
          the system. If a combination already exists, you will not be able to
          save this record and will see a 409 error. In this case, you'll need
