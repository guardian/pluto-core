...
class GeneralDeleteComponent extends React.Component {
  ...
  componentWillMount() {
    axios
      ...
      .then((response) => {
        this.setState(
          { loading: false, selectedItem: response.data.result },
          () => this.postDownload()
        );
      })
      .catch((error) => {
  ...
  confirmClicked(event) {
    axios
      .delete(this.endpoint + "/" + this.props.match.params.itemid)
      .then((response) => {
        if (response.data.status === "warning") {
          this.setState({
            warning: response.data.detail,
    ...
    if (this.state.warning)
      ...
      return (
        <p className="information">
          The following {this.itemClass} will be PERMANENTLY deleted, if you
          click the Delete button below. Do you want to continue?
        </p>
      );
    else return <p className="information" />;
  }
    ...
    if (this.state.warning || this.state.error) {
      return (
        <span style={{ float: "right" }}>
          <button id="cancelButton" onClick={this.cancelClicked}>
            Close
          </button>
        </span>
      );
      ...
      return (
        <span>
          <span style={{ float: "right" }}>
            <button id="deleteButton" onClick={this.confirmClicked}>
              Delete
            </button>
          </span>
