...
class FileReferencesView extends React.Component {
    ...
    this.setState({ loading: true }, () => {
      axios
        ...
        .then((response) => {
          this.setState({
            loading: false,
            projectRefs: response.data.projects,
            templateRefs: response.data.templates,
          });
        })
  ...
  render() {
    ...
    if (
      ...
      this.state.templateRefs.length === 0
    )
    ...
    return (
      <div>
        <h4>Referenced by</h4>
        <ul className="no-gap">
          {this.state.projectRefs.map((ref) => (
            <li>
              <ProjectEntryView
                entryId={ref.id}
                created={ref.created}
          ...
          {this.state.templateRefs.map((ref) => (
            <li>
