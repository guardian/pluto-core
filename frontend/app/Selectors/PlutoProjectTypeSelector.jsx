...
class PlutoProjectTypeSelector extends React.Component {
  ...
  render() {
    const filteredTypes = this.props.subTypesFor
      ? this.props.plutoProjectTypesList.filter(
          (type) => type.parent === this.props.subTypesFor
        )
      : this.props.plutoProjectTypesList.filter(
          (type) => !type.hasOwnProperty("parent") || type.parent === null
        );

    const typesToShow =
      this.props.onlyShowSubtypes && !this.props.subTypesFor
        ? []
        : filteredTypes;
    ...
    return (
      ...
      >
        {typesToShow.concat([{ id: "", name: "(not set)" }]).map((type) => (
