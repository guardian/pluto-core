  ...
  it("should download content when mounted and render a summary", (done) => {
    ...
    const rendered = shallow(
      <GeneralDeleteComponent
        match={{ params: { itemid: 9 } }}
        history={fakeHistory}
      />
    );
    rendered.instance().itemClass =
    ...
    return moxios.wait(() => {
      ...
      request
        .respondWith({
          ...
          response: {
            status: "ok",
            result: { name: "something", data: "something else" },
          },
        })
        .then(() => {
          rendered.update();
          expect(rendered.find("h3").text()).toEqual(
            "Delete long bent thing with a sort of lump on the end"
          );
        ...
        .then(() => {
          ...
          return moxios.wait(() => {
            ...
            deleteRequest
              ...
              .then(() => {
                rendered.update();
                expect(rendered.find("p.warning").text()).toEqual(
