...
describe("Storage SummaryComponent", () => {
  it("should render a summary based on the props given", () => {
    ...
    expect(rendered.find(".login-description").at(3).text()).toEqual(
      "password: "
    );

    expect(rendered.find(".login-value").at(0).html()).toEqual(
      '<span class="login-value"><span>somehostname</span></span>'
    );
    expect(rendered.find(".login-value").at(1).html()).toEqual(
      '<span class="login-value"><span>1234</span></span>'
    );
    expect(rendered.find(".login-value").at(2).html()).toEqual(
      '<span class="login-value"><span>someusername</span></span>'
    );
    expect(rendered.find(".login-value").at(3).html()).toEqual(
      '<span class="login-value"><span class="hidden-password">************</span></span>'
    );
  });
});
